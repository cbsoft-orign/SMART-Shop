...
function runMigrations() {
    // migrations table
    connection_1.db.prepare(`CREATE TABLE IF NOT EXISTS _migrations (
    ...
    const markRun = (name) => {
        connection_1.db.prepare('INSERT INTO _migrations(name, applied_at) VALUES(?, datetime("now"))').run(name);
    };
    // 001 initial schema
    if (!hasRun('001_initial')) {
        ...
        const exists = connection_1.db.prepare('SELECT id FROM users WHERE email = ?').get(adminEmail);
        if (!exists) {
